...
def plot_model_predictions(stock):
    ...
    data = pd.concat([df_foreign, data[['Volume', 'Close', 'Low', 'Open', 'Adj Close', 'High']]],
                     axis=1, sort=True)
    data = data.dropna()
    ...
    names = ['Low', 'Open', 'Adj Close', 'High']
    X_unnorm = scaler.inverse_transform(X[-1])
    X_test = pd.DataFrame(X_unnorm[:, -4:],columns=names)    
    X_test['Date'] = data.index[-(n_steps_in+n_steps_out):-n_steps_out]
    

    y_test = np.reshape(scaler.inverse_transform(y_complete[-1])[:,-4:], (n_steps_out,n_features_out))
    ...
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2,figsize=(9,7), sharex=True,sharey=True)
    ...
    y_pred = pd.DataFrame(scaler.inverse_transform(np.concatenate([y_complete[-1][:,:-4], y_pred], axis=1))[:,-4:],columns=names)
    ...
    y_future = pd.DataFrame(scaler.inverse_transform(np.concatenate([y_complete[-1][:,:-4], y_future], axis=1))[:,-4:],columns=names)
    ...
    y_future = model.predict(np.reshape(dataset[-n_steps_in:], (1, 1, n_steps_in, n_features_in, 1)))
    y_future = np.reshape(y_future[-1], (n_steps_out,n_features_out))
    y_future = pd.DataFrame(scaler.inverse_transform(np.concatenate([y_complete[-1][:,:-4], y_future], axis=1))[:,-4:],columns=names)
    y_future['Date'] = test_date[1:]
