...
def model_conv1_FG(stock):
    ...
    data = pd.concat([data[['Volume', 'Close', 'Low', 'Open', 'Adj Close', 'High']]], axis=1, sort=True)[:-3]
    data = data.dropna()
    ...
    model.fit(X, y, epochs=5000, verbose=0, validation_split=0.2,  callbacks=[early_stop])
...
def model_LSTM_FG(stock):
    ...
    data = pd.concat([ data[['Volume', 'Close', 'Low', 'Open', 'Adj Close', 'High']]],
                     axis=1, sort=True)[:-3]
    data = data.dropna()
