...
#         X_test = np.reshape(dataset[-n_steps_in:, :], (1,n_steps_in,n_features))
#         y = np.array(dataset[-1:, -1:])
        
# # #         forecast_plot(predictions, n_steps_out)

#         global_polarity = 0

    
    
#             (gain_pred, index_maximo_pred) = DLmodels.recommending(predictions, n_steps_out, global_polarity, symbol, accuracy, ML_tech)
...
dados.to_csv('./logs/dados-MAPE.csv')
